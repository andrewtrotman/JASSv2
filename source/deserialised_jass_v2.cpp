...
namespace JASS
	...
	*/
	...
	/*
		...
		-------------------------------------
	*/
	size_t deserialised_jass_v2::read_postings(const std::string &filename)
		{
		/*
			This can take some time so make some noise when we start
		*/
	...
	/*
		...
		----------------------------------
	*/
	size_t deserialised_jass_v2::read_index(const std::string &primary_key_filename, const std::string &vocab_filename, const std::string &terms_filename, const std::string &postings_filename)
		{
	...
	/*
		...
		-----------------------------
	*/
	std::unique_ptr<compress_integer> deserialised_jass_v2::codex(std::string &name, int32_t &d_ness) const
		{
		...
		if (memory == nullptr)
			...
			return serialise_jass_v2::get_compressor(static_cast<serialise_jass_v1::jass_v1_codex>(memory[0]), name, d_ness);
		}
	}
