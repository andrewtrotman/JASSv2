...
namespace JASS
	...
	*/
	...
	class compress_integer_bitpack_128: public compress_integer_bitpack<ACCUMULATOR_TYPE, MAX_DOCUMENTS, MAX_TOP_K>
		{
		public:
			/*
			...
			*/
			virtual size_t encode(void *encoded, size_t encoded_buffer_length, const document_id::integer *source, size_t source_integers)
				{
				return compress_integer_bitpack<ACCUMULATOR_TYPE, MAX_DOCUMENTS, MAX_TOP_K>::encode<128>(encoded, encoded_buffer_length, source, source_integers);
				}
			...
			*/
			static void unittest(void)
				{
				compress_integer<ACCUMULATOR_TYPE, MAX_DOCUMENTS, MAX_TOP_K>::unittest(compress_integer_bitpack_128());
