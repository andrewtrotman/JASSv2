...
/* Definition: PForDelta
   ...
   FAST_DECODE resembles the implementation of Zhang et al; it
	   also won't check for the end of buffer in elementary decode.
...
#define U4D_CODING_VARS I4 b, exc_size, num_exc; I4 *excs;
  ...
  #define P4D_DECODE_ELEM(_X,_B) WORD_CROSS_DECODE(_X,_B)
	...
	#define ENCODE_EXER_START WORD_ENCODE_WRITE
	#define DECODE_EXER_START GET_NEW_WORD
  #define BLK_END_ENCODE	if (__wremaining < WORD_SIZE) WORD_ENCODE_WRITE  
  #define BLK_END_DECODE __wremaining= 0;
  #define NOCONTROL_BITS n*b + ((sum*exc_size+WORD_SIZE-1)/WORD_SIZE)*WORD_SIZE + (WORD_SIZE-24)
#else
  #ifdef FASTER_DECODE
    ...
    #define BLK_END_ENCODE	  
    #define BLK_END_DECODE
  #define NOCONTROL_BITS n*b + ((sum*exc_size+WORD_SIZE-1)/WORD_SIZE)*WORD_SIZE + (WORD_SIZE-16-b)
  #else
    #define P4D_ENCODE_ELEM(_X,_B) WORD_CROSS_ENCODE(_X,_B)
    ...
    #define BLK_END_ENCODE	  
    #define BLK_END_DECODE
		#define NOCONTROL_BITS n*b + sum*exc_size   
	#endif
#endif
...
if (n) {                                                          \
	...
	/*	fprintf(stderr,"b=%d exer=%d first=%d exc_size=%d n=%d\n",b,sum,first_exc,exc_size,n); */ \
		...
		if (__wremaining < WORD_SIZE) ENCODE_EXER_START;                \
		for (i=0,j=0; i<sum; i++) {                                       \
		  /*j += excs[i];  \
			if (j>=n) {    \
			  I4 *p=0;     \
		...
		usedbits= NOCONTROL_BITS;                                 \
		/* forces  writing */                                         \
		BLK_END_ENCODE                                                \
		DIRECT_UPDATE_GROUP(b,16+b,n,0,usedbits)                        \
		BLKSTATS(stderr,b)                                            \
}
